...
function HighlightIcon({ name }: { name: string }) {
  ...
  switch (name) {
    case "waves": return <svg {...base}><path d="M2 6c.6.5 1.2 1 2.5 1C7 7 7 5 9.5 5c2.6 0 2.4 2 5 2 2.5 0 2.5-2 5-2" /><path d="M2 12c.6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2 2.6 0 2.4 2 5 2 2.5 0 2.5-2 5-2" /><path d="M2 18c.6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2 2.6 0 2.4 2 5 2 2.5 0 2.5-2 5-2" /></svg>;
    case "pin": return <svg {...base}><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" /><circle cx="12" cy="10" r="3" /></svg>;
    case "home": return <svg {...base}><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>;
    case "cross": return <svg {...base}><path d="M12 2v20M2 12h20" /></svg>;
    default: return null;
  }
}
  ...
  return (
    <section id="location" className="py-24 md:py-36 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 border border-teal-200 rounded-xl overflow-hidden shadow-sm shadow-teal-50">
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            {highlights.map((h, i) => (
              <div
                key={h.title}
                className={`px-8 md:px-10 py-7 bg-white ${i < highlights.length - 1 ? "border-b border-teal-100" : ""} group`}
              >
                <h4 className="font-black text-[#0f172a] text-sm mb-1.5 flex items-center gap-2.5">
                  <span className="w-6 h-6 bg-teal-50 flex items-center justify-center text-[#14b8a6] group-hover:bg-[#14b8a6] group-hover:text-white transition-colors duration-200 rounded-md">
        ...
        <motion.div
          ...
          className="mt-4 grid grid-cols-2 md:grid-cols-3 rounded-xl overflow-hidden gap-1"
        >
          {[
            { src: "/fotky/river.jpg", alt: "Sázava — výuka u řeky" },
            { src: "/fotky/camp-grounds.jpg", alt: "Areál tábora Březina" },
            { src: "/fotky/river-panorama.jpg", alt: "Koupání na Sázavě" },
          ].map((photo, i) => (
            <div key={photo.src} className={`relative h-52 md:h-64 ${i === 2 ? "hidden md:block" : ""}`}>
              <Image src={photo.src} alt={photo.alt} fill style={{ objectFit: "cover" }} sizes="(max-width: 768px) 50vw, 33vw" />
              <div className="absolute inset-0 bg-[#14b8a6]/10" />
              {i === 0 && (
                <div className="absolute bottom-4 left-5">
                  <span className="bg-white/90 text-[#14b8a6] text-[10px] font-mono tracking-wider px-3 py-1.5 uppercase font-bold rounded-sm">
